 ...
 * 	설계를 하고 설계된 내용으로 문서화를 하는데 실제 개발현장에서 문서화 된 설계내용은
 * 	잘 지켜지지 않는 경우가 많다.
 * 
 * 그러한 상황이 많아지면 여러 클래스가 연동되는 프로젝트의 경우, 특히 method 의 이름을
 * 	잘못 지어(다르게 지어) 코드 연결(연동)이 어려워 진다.
 * 
 * 인터페이스는 여러 개발자들이 협업을 할때 클래스, method 규칙을 코드로 구현하여 
 * 	클래스를 만들때 반드시 implements 하도록 강제한다.
 * 
 * 인터페이스를 impl 한 클래스는 인터페이스에 설계된 추상메서드를 반드시 자신이 만든 
 *	클래스에서 구현해야 한다. 그러한 규칙을 강제 함으로써 연결, 연동의 혼란을 최소화 한다.
 *
 * 
 * 
