package com.inqu0302.standard.impl;
...
/*
 * 인터페이스를 implements 한 클래스는 이름을 지을때 접미사에 Impl을 붙인다.
 * 
 * 인터페이스를 impl 한 클래스는 인터페이스에 설계된 추상메서드를 "반드시"
 * 	구체적인 코드로 구현해야 한다.
 * 
 * 만약 아직 구체적인 코드가 구현되지 않았더라도 method의 형태는 삭제할 수 없다.
 * 
 * 이러한 규칙을 적용하여 다른 클래스와 연동할떄 편리하고, 오류가 없도록 하기 위함
 */
public class InputServiceImplV1 implements InputService{
// 메서드 생성시 해당 규격을 따르지 않으면 사용이 불가
	...
	public Integer inputValue(String title) {
		...
		while(true) {
			...
			try {
				...
				System.out.println("정수나 QUIT 만 입력가능!");
				continue;
			}
			return intIn;
		}// end while
