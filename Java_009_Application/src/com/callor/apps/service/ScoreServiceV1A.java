...
public class ScoreServiceV1A {
	...
	// Exception 클래스를 사용하여 객체를 " 선언만 " 하고 아직 사용할 준비가
	// 되어있지 않다.
	...
	// Service 클래스에서 배열의 개수를 고정하지 말고 최초로 객체를 생성할때 
	// 필요한 개수를 매개변수로 전달하고 전달받은 매개변수(members)의 값을 사용하여
	// 배열을 만든다
	public ScoreServiceV1A(int members) {
		// scan 객체를 생성(초기화)하기 객체를 선언하고 생성하는 코드가 없으면
	...
	public void addNum() {
		...
		/*
		 * inputScore() method는 private으로 선언되어 있기 때문에 객체.inputScore()
		 * 형식으로 사용할 수 없다. 
		 */
//		this.inputScore();
	...
	// private : 개인적인
	// 현재 클래스에서만 method를 호출하여 명령을 실행할 수 있도록 하는 keyword
	// 객체.inputScore() 로 사용이 불가하다
	// 외부에 공개하지 않고, 현재 클래스의 코드에서만 호출할 수 있도록
	// 제한하는 keyword
	// integer : void keyword 대신 정수를 선언하는 키워드를 사용해 method 선언하기
	// method 코드 끝에 반드시 return 명령문이 있어야 한다.
	
	// 여기에서 사용된 Integer 키워드는 변수를 선언하는 용도의 키워드가 아니고
	// return type 키보드라고 한다.
	// method의 코드내에 return 값 형식의 코드가 반드시 있어야 함을 선언하는 것임
	//	retur type 이 void 형(type)일 경우는 retunr 명령문이 생략되거나,
	// return 명령문만 단독으로 사용할 수 있다.
	
	// return type 이 void가 아닌경우 return 명령문을 \반드시 코드내에 적정할 위치에
	// 있어야 하고 return 값 형식으로 사용해야 한다.
