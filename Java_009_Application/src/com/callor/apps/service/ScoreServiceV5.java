...
public class ScoreServiceV5 {
	 ...
	 * 몇명의 학생, 일부과목 점수만 입력을 마치고 잠시 업무를 멈추고 싶을때
	 * 현재 코드는 불가능하다
	...
	public void makeScores() {
		...
		for(int i = 0 ; i < intKor.length ; i++) {
			System.out.printf("%d번 학생의 점수를 입력\n",i+1);
			for(int j = 0; j < subject.length ; i++) {
				scores[j] = this.inputScore(subject[j]);
				
				/*
				 * 입력업무중에 -1을 입력했을때 입력 업무를 중단하고 main() 으로 되돌아가고 싶다. 
				 * 이때 break를 사용하면 for()가 겹쳐있기 때문에 원하는대로 코드가 작동되지 않는다.
				 * 이럴경우 return type 이 void 이지만 return 명령을 실행하여
				 */
				if(scores[j] == -1) {
					return;
				}
	...
	private Integer inputScore(String sub) {
		...
		while(true) {
			...
			// 점수 입력을 하는 중에 -1 을 입력하면 입력하는 업무를 중단 
...
//				continue;
	 ...
	 * 이 클래스를 객체로 선언하고, 생성한 후에 여러가지 method를 
	 * 호출하여 업무를 수행할 수 있다.
	 * 
	 * method나 변수에 static을 부착하면 사용하지 않아도 method 코드나, 변수가 메모리에
	 * 상주하게 된다.
	 */
