...
public class Class_02 {
	...
	public static void main(String[] args) {
		 ...
		 * 					선언을 할때는 List를 사용하고 생성(초기화)할때는
		 * 					ArrayList() 생성자를 사용
		 * 
		 * List 형 객체를 선언하고 ArrayList()로 생성을 하면 size가 0인 배열
		 * 	객체가 생성되는 것과 유사
		 */
		...
		// 배열에 값을 저장할때는 원하는 length 만큼 배열을 선언 생성하고 첨자를 사용하여
		// 위치를 지정한 후 값을 저장
		int[] nums = new int[5];
		nums[0] = 100;
		
		//List 형 객체에 데이터를 저장할 때는 "추가" 를 하여 저장
		// List형 객체에 데이터를 추가할때는 .add() method 사용하여 데이터를 추가한다.
		//intList의 size가 1이되고 자동으로 0번위치에 100이 저장
		intList.add(100); // 0번 위치 
		intList.add(200); // 1번 위치
		intList.add(300); // 2번 위치
		
		// intList의 0번 위치에 저장된 값으르 읽고 싶다.
		System.out.println(intList.get(0));
		System.out.println(intList.get(1));
		System.out.println(intList.get(2));
		
		//  값을 읽을때 저장된 데이터 개수의 index범위를 벗어날 수 없다.
		// 위에서 add() 3번 실행했기 때문에 size는 3이 되고 get(index)를 수행할때
