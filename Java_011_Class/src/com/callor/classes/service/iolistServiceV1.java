 ...
 * Java에서는 배열을 사용해야 하는 프로젝트에서 List클래스를
 * 사용한 객체를 생성하여 진행을한다.
 */
public class iolistServiceV1 {
	/*
	 * 매입매출 데이터들을 사용하여 연산을 수행
	 * 매입매출 데이터들의 묶음을 List 형 객체로 사용하겠다.
	 * 
	 * List형 객체에 포함된 (개별)데이터는 IolistVO 형으로 사용하겠다
	 * 
	 * 낱개 데이터는 IolistVO 클래스 형으로 사용하고 그 데이터들을
	 * 묶어서 list 형으로 연산수행을 하겠다.
	 * 
	...
	// public 으로 선언된 method는 객체.method()형식으로 호출이가능
	// return type이 명확히 결정되지 않았을때는 void형으로 작성
