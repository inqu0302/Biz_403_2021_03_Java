...
public class Method_01 {
	...
	public static void main(String[] args) {
		 ...
		 * 객체를 생성(초기화)하기
		 * 객체를 선언만 하고 생성(초기화)을 하지 않고 객체.method() 를 호출하면 
		...
		/*
		 * Scanner 클래스를 사용하여 scan 객체를 선언하고 Scanner()생성자 method를 호출하여
		 * 	scan 객체를 생성한다.
		 * Scanner () 생성자는 객체를 생성할때 System.in(키보드 입력 장치)를 연결할수 있도록
		 * 	매개변수에 정보를 전달해주어야 한다.
		 */
		 ...
		 * 사용법이 클래스 방식이면서 일반적인 변수처럼 사용할수 있다.
		 ...
		 * 사용하는 기본방법은 동일하다
		 * Integer는 클래스 적인 특성을 갖고 있다보니 new Integer() 생성자를 사용하여 변수를 
		 * 	만들면 다소 엉뚱하게 작동되는 경우가 있다.
		...
		/*
		 * if 조건문을 사용하여 어떤 조건을 검사했을때 조건에 완전히 일치하는 경우에는 
		 * 	~~하다. ~~ 다 라고 표현하고 조건에 일치하지 않는 else를 사용하여 표현할때는
		 * 	딱 맞아 떨어지는 조건을 찾기가 어렵기때문에 ~~가 아니다 라고 표현하는것이 좋다
		 */
		if(int1 % 3 ==0) {
		...
		/*
		 * 어떤수가 짝수인가를 조건검사하고 짝수이면 짝수이다 라고 표현하고 else이면 홀수이다라고
		 * 	표현할수 있지만 코드를 작성할때 조건으로 명시한 이외의 경우는 가급적 ~~이 아니다 라고 
		 * 	표현하자
		 */
		if (int1 % 2 == 0) {
