...
/*
 * 전통적인(구조적 프로그래밍) 방법에서는 
 * 같은 클래스에서 같은 이름의 method 는 중복하여 작성할수 없다.
 * 
 * inputValue1("새우깡);
 * inpubValue2("새우깡",1000);
 * inpubValue3("새우깡",0,1000);
 * inpubValue2("새우깡",0,1000); // 매개변수 오류발생
 * public Integer inputValue1(String title);
 * public Integer inputValue2(String title, int start);
 * public Integer iunputValue3(String title, int start, int end);
 * 
 * 객체지향 개발 방법에서는
 * 매개변수의 개수, 타입, 순서가 다르면 같은 이름의 method를 
 * 중복하여 작성 할 수 있다.
 * 
 * public Integer inputValue(String title); //1
 * public Integer inputValue(String title, int start); //2
 * public Integer iunputValue(String title, int start, int end); //3
 * 
 * 객체.inputValue("새우깡",1000); // 자동으로 2번 method 가 연결, 호출
...
public class InputServiceV2 {
	...
	public Integer inputValue(String title, int start, int end) {
		...
		while(true) {
			Integer intNum = this.inputValue(msgTitle);
			// this.inputValue(String title) method가 return 해준 값이
			// null 이 아니면 유효성 검사 수행
			if(intNum != null) {
				// 유효성 검사를 통과하지 못하면
			...
			// inputValue(String title)이 null을 return 했거나 유효한 범위의
			// 정수를 return 했으면 그 값이 담겨 있다.
			return intNum;
	...
	public Integer inputValue(String title) {
		/*
		 * exception이 발생했을때 사용자에게 안내 메시지를 보여주고
		 * 다시 입력을 받도록 하기위한 선 조치
		 */
		while(true) {
			...
			// if 조건문이 false 일경우 실행되는 부분
			// 굳이 else를 사용하지 않아도 문제가 없다
			...
			// try-catch block 의 코드가 정상적으로 모두 완료되었을때
			// 실행되는 부분
			// 입력한 값이 정상적으로 정수형 데이터로 변환되어 intNum 변수에
			// 담겨 있다.
			// 입력한 정수값이 return 된다.
			return intNum;
		}
