...
public class FileWriter_02 {
	...
	public static void main(String[] args) {
		...
		try {
			/*
			 * PrintWrite 클래스만으로도 파일에 데이터를 기록할 수 있다
			 * 그럼에도 불구하고 FileWriter를 사용하는 이유는 FilewWriter 생성자의
			 * 두번째 파라메터 때문이다
			 * 	new FileWriter(파일이름, bAppend)
			 * 
			 * FileWriter 객체의 두번째 파라메터를 true로 설정하면 파일을 
			 * 	Append Mode로 Open한다.
			 * 
			 * 두번째 파라메터가 없거나 false로 설정하면(값을 설정하지 않으면 기본값은 false)
